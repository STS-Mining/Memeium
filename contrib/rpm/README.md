...
The RPM spec file provided here is for Memeium-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://memeium.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Memeium git repository](https://github.com/STS-Mining/Memeium) but are not part of
...
Currently only `Source10` is used, for the example `memeium.conf` file.
...
in the memeium git repository. At present only an SVG version of the Memeium
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the memeium
...
needed when 0.12.1 is released, a proper fix is already in the Memeium git
master branch.
...
idea to build Memeium Core against a static version of that library compiled
...
The `memeium.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `memeium-server` package. At this point, most RPM based
...
The `memeium-server` package should have SELinux support. How to properly do
...
Memeium-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/memeium/). He has been
packaging Memeium for Fedora at least since 2012.
...
4. I renamed the `memeium` package that contains the Qt GUI to `memeium-core` as
that appears to be how the general population refers to it, in contrast to
`memeium-xt` or `memeium-classic`. I wanted to make sure the general population
