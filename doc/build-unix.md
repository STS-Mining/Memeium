...
====================
Some notes on how to build Memeium Core in *nix.


Note
---------------------
Always use absolute paths to configure and compile memeium and the dependencies,
...
This will build memeium-qt as well if the dependencies are met.
...
Notes
-----
The release is built with GCC and then "strip memeiumd" to strip the debug
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, memeium should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, memeium may be compiled in
...
`--with-incompatible-bdb` according to the [PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/memeium/trunk/PKGBUILD).
As mentioned above, when maintaining portability of the wallet between the standard Memeium Core distributions and independently built
