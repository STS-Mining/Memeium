...
class MemeiumTestFramework:
    """Base class for a memeium test script.

    Individual memeium test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true", help="Leave memeiumds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true", help="Don't stop memeiumds after the test execution")
        ...
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"), help="Source directory containing memeiumd/memeium-cli (default: %default)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: memeiumd's were not stopped and may still be running")
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2 ** 16) as log_stderr:
            try:
                ...
                assert 'memeiumd exited' in str(e)  # node must have shutdown
    ...
    def _start_logging(self):
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("MemeiumRPC")
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            for i in range(MAX_NODES):
                ...
                args = [os.getenv("MEMEIUMD", "memeiumd"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0"]
                if i > 0:
        ...
        for i in range(self.num_nodes):
            ...
            initialize_data_dir(self.options.tmpdir, i)  # Overwrite port/rpcport in memeium.conf
