...
`test/functional/test_framework/test_framework.py:MemeiumTestFramework.main`.
...
def main():
    ...
    # Check that the build was configured with wallet, utils, and memeiumd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_memeiumd = config["components"].getboolean("ENABLE_MEMEIUMD")
    if not (enable_wallet and enable_cli and enable_memeiumd):
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    ...
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "memeiumd"]) is not None:
    ...
    #Set env vars
    if "MEMEIUMD" not in os.environ:
        os.environ["MEMEIUMD"] = build_dir + '/src/memeiumd' + exeext
        os.environ["MEMEIUMCLI"] = build_dir + '/src/memeium-cli' + exeext
