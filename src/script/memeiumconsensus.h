...
#ifndef MEMEIUM_MEMEIUMCONSENSUS_H
#define MEMEIUM_MEMEIUMCONSENSUS_H

#include <stdint.h>

#if defined(BUILD_MEMEIUM_INTERNAL) && defined(HAVE_CONFIG_H)
...
#endif
#elif defined(MSC_VER) && !defined(STATIC_LIBMEMEIUMCONSENSUS)
...
#define MEMEIUMCONSENSUS_API_VER 1
...
typedef enum memeiumconsensus_error_t {
    ...
    memeiumconsensus_ERR_AMOUNT_REQUIRED,
...
enum {
    ...
    memeiumconsensus_SCRIPT_FLAGS_VERIFY_WITNESS = (1U << 11),             // enable WITNESS (BIP141)
    memeiumconsensus_SCRIPT_FLAGS_VERIFY_ALL = memeiumconsensus_SCRIPT_FLAGS_VERIFY_P2SH | memeiumconsensus_SCRIPT_FLAGS_VERIFY_DERSIG |
                                               ...
                                               memeiumconsensus_SCRIPT_FLAGS_VERIFY_CHECKSEQUENCEVERIFY | memeiumconsensus_SCRIPT_FLAGS_VERIFY_WITNESS
};
...
EXPORT_SYMBOL int memeiumconsensus_verify_script(const unsigned char* scriptPubKey, unsigned int scriptPubKeyLen, const unsigned char* txTo, unsigned int txToLen, unsigned int nIn, unsigned int flags, memeiumconsensus_error* err);

EXPORT_SYMBOL int memeiumconsensus_verify_script_with_amount(const unsigned char* scriptPubKey, unsigned int scriptPubKeyLen, int64_t amount, const unsigned char* txTo, unsigned int txToLen, unsigned int nIn, unsigned int flags, memeiumconsensus_error* err);
...
#endif // MEMEIUM_MEMEIUMCONSENSUS_H
