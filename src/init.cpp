 ...
 * between memeiumd, and memeium-qt and non-server tools.
 */
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GenerateMemeiums(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), MEMEIUM_CONF_FILENAME));
    if (mode == HMM_MEMEIUMD) {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), MEMEIUM_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/STS-Mining/Memeium>";
...
/** Sanity checks
 *  Ensure that Memeium is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Memeium process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", MEMEIUM_CONF_FILENAME)).string());
