...
bool TransferAssetFromScript(const CScript& scriptPubKey, CAssetTransfer& assetTransfer, std::string& strAddress)
{
    ...
    if (AreTransferScriptsSizeDeployed()) {
        ...
        // This fixes that issue (https://github.com/STS-Mining/Memeium/issues/752)
...
{
    // Must contain at least 3 outpoints (MMM burn, owner change and one or more new unique assets that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
{
    // Issuing an Asset must contain at least 3 CTxOut( Memeium Burn Tx, Any Number of other Outputs ..., Owner Asset Tx, New Asset Tx)
    if (vout.size() < 3) {
...
{
    // Issuing an Asset must contain at least 2 CTxOut( Memeium Burn Tx, New Asset Tx, Any Number of other Outputs...)
    if (vout.size() < 2) {
...
{
    // Reissuing an Asset must contain at least 3 CTxOut ( Memeium Burn Tx, Any Number of other Outputs ..., Reissue Asset Tx, Owner Asset Change Tx)
    if (vout.size() < 3) {
...
{
    // Check the first transaction and verify that the correct MMM Amount
...
{
    // Verify 'MMMq' is in the transaction
...
{
    // Verify 'MMMr' is in the transaction
    CScript scriptPubKey = txOut.scriptPubKey;
