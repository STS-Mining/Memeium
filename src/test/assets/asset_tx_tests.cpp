...
BOOST_AUTO_TEST_CASE(asset_tx_valid_test)
{
    ...
    CAssetTransfer asset("MEMEIUMTEST", 1000);
...
BOOST_AUTO_TEST_CASE(asset_tx_not_valid_test)
{
    ...
    // This should fail because 900 MEMEIUM doesn't have a destination
    CAssetTransfer assetTransfer("MEMEIUMTEST", 100);
...
BOOST_AUTO_TEST_CASE(asset_tx_valid_multiple_outs_test)
{
    ...
    for (int i = 0; i < 10; i++) {
        CAssetTransfer asset2("MEMEIUMTEST", 100);
...
BOOST_AUTO_TEST_CASE(asset_tx_multiple_assets_test)
{
    ...
    CAssetTransfer asset2("MEMEIUMTESTTEST", 1000);
    CScript scriptPubKey2 = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
    asset2.ConstructTransaction(scriptPubKey2);

    CAssetTransfer asset3("MEMEIUMTESTTESTTEST", 1000);
    ...
    for (int i = 0; i < 10; i++) {
        // Add the first asset
        CAssetTransfer outAsset("MEMEIUMTEST", 100);
        ...
        CAssetTransfer outAsset2("MEMEIUMTESTTEST", 100);
        ...
        // Add the third asset
        CAssetTransfer outAsset3("MEMEIUMTESTTESTTEST", 100);
    ...
    // The inputs are spending 3000 Assets (1000 of each MEMEIUM, MEMEIUMTEST, MEMEIUMTESTTEST)
    // The outputs are spending 100 Assets to 10 destinations (10 * 100 = 1000) (of each MEMEIUM, MEMEIUMTEST, MEMEIUMTESTTEST)
