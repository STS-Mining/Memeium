...
BOOST_AUTO_TEST_CASE(name_validation_tests)
{
    ...
    //- Versions of MEMEIUM NOT allowed
    BOOST_CHECK(!IsAssetNameValid("MMM", type));
    BOOST_CHECK(!IsAssetNameValid("MEMEIUM", type));

    //- Versions of MEMEIUM ALLOWED
    BOOST_CHECK(IsAssetNameValid("MEMEIUM.COIN", type));
    BOOST_CHECK(IsAssetNameValid("MEMEIUM_COIN", type));
    BOOST_CHECK(IsAssetNameValid("MMMSPYDER", type));
    BOOST_CHECK(IsAssetNameValid("SPYDERMMM", type));
    BOOST_CHECK(IsAssetNameValid("MEMEIUMSPYDER", type));
    BOOST_CHECK(IsAssetNameValid("SPYDEMEMEIUM", type));
    BOOST_CHECK(IsAssetNameValid("BLACK_MEMEIUMS", type));
    BOOST_CHECK(IsAssetNameValid("SEMMMOT", type));
...
BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
{
    ...
    CAssetTransfer asset("MEMEIUM", 1000);
...
BOOST_AUTO_TEST_CASE(new_asset_coin_test)
{
    ...
    // Create the asset scriptPubKey
    CNewAsset asset("MEMEIUM", 1000, 8, 1, 0, "");
    CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
    asset.ConstructTransaction(scriptPubKey);
