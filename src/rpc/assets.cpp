...
{
    return AreAssetsDeployed() ? "" : "\nTHIS COMMAND IS NOT YET ACTIVE!\nhttps://github.com/STS-Mining/rips/blob/master/rip-0002.mediawiki\n";
}
...
UniValue issue(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "4. \"change_address\"        (string), optional, default=\"\"), address the the mmm change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue issueunique(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "5. \"change_address\"        (string, optional, default=\"\"), address the the mmm change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue transfer(const JSONRPCRequest& request)
{
    ...
    std::string mmm_change_address = "";
    if (request.params.size() > 5) {
        mmm_change_address = request.params[5].get_str();
    }
    ...
    CTxDestination mmm_change_dest = DecodeDestination(mmm_change_address);
    if (!mmm_change_address.empty() && !IsValidDestination(mmm_change_dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("MMM change address must be a valid address. Invalid address: ") + mmm_change_address);
    ...
    CCoinControl ctrl;
    ctrl.destChange = mmm_change_dest;
...
UniValue transferfromaddresses(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
            "transferfromaddresses \"asset_name\" [\"from_addresses\"] qty \"to_address\" \"message\" expire_time \"mmm_change_address\" \"asset_change_address\"\n" + AssetActivationWarning() +
            ...
            "7. \"mmm_change_address\"       (string, optional, default = \"\") the transactions MMM change will be sent to this address\n"
    ...
    std::string mmm_change_address = "";
    if (request.params.size() > 6) {
        mmm_change_address = request.params[6].get_str();
    }
...
UniValue transferfromaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
            "transferfromaddress \"asset_name\" \"from_address\" qty \"to_address\" \"message\" expire_time \"mmm_change_address\" \"asset_change_address\"\n" + AssetActivationWarning() +
            ...
            "5. \"message\"                  (string, optional) Once RIP5 is voted in ipfs hash or txid hash to send along with the transfer\n"
            "6. \"expire_time\"              (numeric, optional) UTC timestamp of when the message expires\n"
            "7. \"mmm_change_address\"       (string, optional, default = \"\") the transaction MMM change will be sent to this address\n"
...
UniValue issuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 4 || request.params.size() > 9)
        throw std::runtime_error(
            ...
            "5. \"change_address\"        (string, optional, default=\"\") address that the mmm change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue reissuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
            ...
            "6. \"change_address\"        (string, optional, default=\"\") address that the mmm change will be sent to, if it is empty, change address will be generated for you\n"
...
#ifdef ENABLE_WALLET
        {"assets", "transferfromaddress", &transferfromaddress, {"asset_name", "from_address", "qty", "to_address", "message", "expire_time", "mmm_change_address", "asset_change_address"}},
        {"assets", "transferfromaddresses", &transferfromaddresses, {"asset_name", "from_addresses", "qty", "to_address", "message", "expire_time", "mmm_change_address", "asset_change_address"}},
