    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            CURRENCY_UNIT + " to send. eg 0.1\n"
                            ...
                            "                             The recipient will receive less memeiums than you enter in the amount field.\n"
...
UniValue sendfromaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\nSend an amount from a specific address to a given address. All mmm change will get sent back to the from_address\n" +
            HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            "1. \"from_address\"       (string, required) The Memeium address to send from.\n"
            "2. \"to_address\"            (string, required) The Memeium address to send to.\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    StartShutdown();
    return "wallet encrypted; Memeium server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
{
    throw std::runtime_error("bumpfee has been deprecated on the MMM Wallet.");
