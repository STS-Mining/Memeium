...
const std::string MemeiumGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
    "macosx"
...
const QString MemeiumGUI::DEFAULT_WALLET = "~Default";

/* Bit of a bodge, c++ really doesn't want you to predefine values
 * in only header files, so we do one-time value assignment here. */
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = {{{"BTC", "MMMBTC", 1, 8},
    {"mBTC", "MMMBTC", 1000, 5},
    {"µBTC", "MMMBTC", 1000000, 2},
    {"Satoshi", "MMMBTC", 100000000, 0},
    {"USDT", "MMMUSDT", 1, 5}}};

static bool ThreadSafeMessageBox(MemeiumGUI* gui, const std::string& message, const std::string& caption, unsigned int style);

MemeiumGUI::MemeiumGUI(const PlatformStyle* _platformStyle, const NetworkStyle* networkStyle, QWidget* parent) : QMainWindow(parent),
                                                                                                                 enableWallet(false),
...
void MemeiumGUI::createActions()
{
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to MMM addresses"));
...
void MemeiumGUI::createToolBars()
{
    if (walletFrame) {
        QSettings settings;
        ...
        if (IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/mmmtext")));
        } else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/memeiumtext")));
        }
        ...
        QVBoxLayout* memeiumLabelLayout = new QVBoxLayout(toolbarWidget);
        memeiumLabelLayout->addWidget(labelToolbar);
        memeiumLabelLayout->addWidget(m_toolbar);
        memeiumLabelLayout->setDirection(QBoxLayout::TopToBottom);
        memeiumLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Memeium Market Price"));
        ...
        comboMmmUnit = new QComboBox(headerWidget);
        QStringList list;
        for (int unitNum = 0; unitNum < CurrencyUnits::count(); unitNum++) {
            list.append(QString(CurrencyUnits::CurrencyOptions[unitNum].Header));
        }
        comboMmmUnit->addItems(list);
        comboMmmUnit->setFixedHeight(26);
        comboMmmUnit->setContentsMargins(5, 0, 0, 0);
        comboMmmUnit->setStyleSheet(STRING_LABEL_COLOR);
        comboMmmUnit->setFont(currentMarketFont);

        labelVersionUpdate->setText("<a href=\"https://github.com/STS-Mining/Memeium/releases\">New Wallet Version Available</a>");
        ...
        priceLayout->addWidget(comboMmmUnit, 0, Qt::AlignBottom | Qt::AlignLeft);
        ...
        connect(comboMmmUnit, SIGNAL(activated(int)), this, SLOT(currencySelectionChanged(int)));
        // Create the timer
        ...
        // Get the latest Memeium release and let the user know if they are using the latest version
        // Network request code for the header widget
                    ...
                    if (key == "tag_name") {
                        ...
                        if (fNewSoftwareFound) {
                            ...
                            if (GetRandInt(2) == 1) {
                                ...
                                if (fRet) {
                                    QString link = "https://github.com/STS-Mining/Memeium/releases";
...
void MemeiumGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if (IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/mmmtext")));
        m_toolbar->setMaximumWidth(65);
...
void MemeiumGUI::setClientModel(ClientModel* _clientModel)
{
...
#ifdef ENABLE_WALLET
bool MemeiumGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    if (!walletFrame)
        return false;
...
bool MemeiumGUI::setCurrentWallet(const QString& name)
{
    if (!walletFrame)
        return false;
...
void MemeiumGUI::removeAllWallets()
{
    if (!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MemeiumGUI::setWalletActionsEnabled(bool enabled)
{
...
void MemeiumGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void MemeiumGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void MemeiumGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MemeiumGUI::optionsClicked()
{
...
void MemeiumGUI::showDebugWindow()
{
...
void MemeiumGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void MemeiumGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void MemeiumGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void MemeiumGUI::openClicked()
{
...
void MemeiumGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MemeiumGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MemeiumGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MemeiumGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MemeiumGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MemeiumGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** MMM START */
void MemeiumGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void MemeiumGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void MemeiumGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void MemeiumGUI::gotoRestrictedAssetsPage()
{
...
void MemeiumGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Memeium network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void MemeiumGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MemeiumGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void MemeiumGUI::updateHeadersSyncProgressLabel()
{
...
void MemeiumGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay) {
        if (header)
...
void MemeiumGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
    QString strTitle = tr("Memeium"); // default title
...
void MemeiumGUI::changeEvent(QEvent* e)
{
...
void MemeiumGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if (clientModel && clientModel->getOptionsModel()) {
...
void MemeiumGUI::showEvent(QShowEvent* event)
{
...
#ifdef ENABLE_WALLET
void MemeiumGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "MMM")
        msg += tr("Amount: %1\n").arg(MemeiumUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(MemeiumUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void MemeiumGUI::dragEnterEvent(QDragEnterEvent* event)
{
    // Accept only URIs
...
void MemeiumGUI::dropEvent(QDropEvent* event)
{
    if (event->mimeData()->hasUrls()) {
...
bool MemeiumGUI::eventFilter(QObject* object, QEvent* event)
{
...
#ifdef ENABLE_WALLET
bool MemeiumGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MemeiumGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void MemeiumGUI::setEncryptionStatus(int status)
{
    switch (status) {
...
void MemeiumGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void MemeiumGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MemeiumGUI::detectShutdown()
{
...
void MemeiumGUI::showProgress(const QString& title, int nProgress)
{
    if (nProgress == 0) {
...
void MemeiumGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon) {
...
void MemeiumGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(MemeiumGUI* gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void MemeiumGUI::subscribeToCoreSignals()
{
...
void MemeiumGUI::unsubscribeFromCoreSignals()
{
...
void MemeiumGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void MemeiumGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
                                                                                               ...
                                                                                               menu(0)
{
    ...
    QList<MemeiumUnits::Unit> units = MemeiumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MemeiumUnits::Unit unit : units) {
#ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(MemeiumUnits::name(unit)));
#else
        max_width = qMax(max_width, fm.width(MemeiumUnits::name(unit)));
#endif
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle* platformStyle)
{
    ...
    for (MemeiumUnits::Unit u : MemeiumUnits::availableUnits()) {
        QAction* menuAction = new QAction(QString(MemeiumUnits::name(u)), this);
...
{
    setText(MemeiumUnits::name(newUnits));
}
...
void MemeiumGUI::currencySelectionChanged(int unitIndex)
{
    if (clientModel && clientModel->getOptionsModel()) {
...
void MemeiumGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "MemeiumGUI::onPriceUnitChange: " + QString::number(newIndex);
    ...
    this->comboMmmUnit->setCurrentIndex(newIndex);
    this->getPriceInfo();
}

void MemeiumGUI::getPriceInfo()
{
...
void MemeiumGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/STS-Mining/Memeium/releases"));
