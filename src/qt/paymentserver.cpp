...
const int MEMEIUM_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString MEMEIUM_IPC_PREFIX("memeium:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++) {
        ...
        if (arg.startsWith(MEMEIUM_IPC_PREFIX, Qt::CaseInsensitive)) // Memeium: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseMemeiumURI(arg, &r) && !r.address.isEmpty()) {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests) {
        ...
        if (!socket->waitForConnected(MEMEIUM_IPC_CONNECT_TIMEOUT)) {
            delete socket;
            socket = nullptr;
            return false;
        }
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(MEMEIUM_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(MEMEIUM_IPC_PREFIX, Qt::CaseInsensitive)) // memeium: URI
    {
        ...
        } else // normal URI
        {
            ...
            if (GUIUtil::parseMemeiumURI(s, &recipient)) {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (IsDust(txOut, ::dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).").arg(MemeiumUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
